...
describe('testing <Pie /> chart', () => {
  ...
  it('<Pie /> chart return null if data is null', async () => {
    const dimension: Field[] = [
      {
        fieldKey: 'd1',
        fieldName: '日期'
  ...
  it('getCenter() works fine while legendPosition changed', async () => {
    const data = [
      {
        v6: 0.8141021277904137,
        d1: '2020-12-31',
    ...
    let instanceTop: any
    let instanceLeft: any
    let instanceRight: any
    let instanceBottom: any
    ...
    expect(instanceTop).toBeDefined()
    expect(instanceTop.getEchartsInstance()).toBeDefined()
    const seriesTop = instanceTop.getEchartsInstance().getOption().series
    expect(seriesTop[0].center).toEqual(['50%', '55%'])

    expect(instanceLeft).toBeDefined()
    expect(instanceLeft.getEchartsInstance()).toBeDefined()
    const seriesLeft = instanceLeft.getEchartsInstance().getOption().series
    expect(seriesLeft[0].center).toEqual(['55%', '50%'])

    expect(instanceRight).toBeDefined()
    expect(instanceRight.getEchartsInstance()).toBeDefined()
    const seriesRight = instanceRight.getEchartsInstance().getOption().series
    expect(seriesRight[0].center).toEqual(['45%', '50%'])

    expect(instanceBottom).toBeDefined()
    expect(instanceBottom.getEchartsInstance()).toBeDefined()
    const seriesBottom = instanceBottom.getEchartsInstance().getOption().series
    expect(seriesBottom[0].center).toEqual(['50%', '45%'])
  })
})
