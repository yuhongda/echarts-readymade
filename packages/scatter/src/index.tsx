  ...
  if (_chartOption) {
    if (compareDimension && compareDimension.length > 0) {
      ...
      compareDimensionValues = [
        ...new Set<string>(
          _data.map(
            (d: any) => d[_dimensionListCompare[0] && _dimensionListCompare[0].fieldKeyAlias]
          )
        )
      ]
      ...
      _data.forEach((d: any) => {
        const _index = _preProcessData.findIndex(
          (item) => item.name == d[_dimension[0] && _dimension[0].fieldKeyAlias]
        )
        if (_index != -1) {
          _preProcessData[_index].data.push(d)
        } else {
          _preProcessData.push({
            name: d[_dimension[0] && _dimension[0].fieldKeyAlias],
            data: [d]
          })
        }

        maxX = Math.max(d[_valueListX[0].fieldKeyAlias] || 0, maxX)
        maxY = Math.max(d[_valueListY[0].fieldKeyAlias] || 0, maxY)
      })

      if (
        source.chartData.chartOption.scatterQuadrant &&
        source.chartData.chartOption.scatterQuadrant.showTop &&
        source.chartData.chartOption.scatterQuadrant.top
      ) {
        // 截取last top
        if (source.chartData.chartOption.scatterQuadrant.reverse) {
          _preProcessData.reverse()
        }

        _preProcessData = _preProcessData.slice(0, source.chartData.chartOption.scatterQuadrant.top)
        ...
        if (
          source.chartData.chartOption.scatterQuadrant &&
          source.chartData.chartOption.scatterQuadrant.reverse
        ) {
          _preProcessData.reverse()
        }

        _preProcessData = _preProcessData.slice(0, 10)
        ...
        return {
          ...
          data: _preProcessData.map((pd) => {
            const v = pd.data.find(
              (d) => d[_dimensionListCompare[0] && _dimensionListCompare[0].fieldKeyAlias] == item
            )
            if (v) {
              return v
            } else {
              const _v: { [key: string]: string } = {}
              _v[_dimension[0] && _dimension[0].fieldKeyAlias] = pd.name
              return _v
            }
          })
        }
