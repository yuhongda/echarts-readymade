...
export const Stack = (props) => {
    const { ref, context, dimension, compareDimension, valueList, echartsSeries, setOption, xAxisData, isPercentMode, isLineStack, ...restSettings } = props;
    ...
    if (_chartOption) {
        ...
        if (compareDimension && compareDimension.length > 0) {
            ...
            // 按维度分组
            data?.forEach((d) => {
                const _index = _preProcessData.findIndex((item) => item.name == (_dimension && d[_dimension[0].fieldKey]));
                if (_index != -1) {
                    const _indexCompareDimensionData = _preProcessData[_index].data.findIndex((item) => item[compareDimension[0] && compareDimension[0].fieldKey] ==
            ...
            try {
                _preProcessData = _preProcessData.sort((a, b) => {
                    const sumA = a.data.reduce((s, c) => {
                        s = s + (c[_valueItem.fieldKey] || 0);
                        return s;
                    }, 0);
                    const sumB = b.data.reduce((s, c) => {
                        s = s + (c[_valueItem.fieldKey] || 0);
                        return s;
                    }, 0);
                    return sumB - sumA;
            ...
            sumData = _preProcessData.map((item, i) => {
                return {
                    name: item.name,
                    value: item.data.reduce((s, c) => s + c[_valueItem.fieldKey], 0)
                };
            });
            _chartOption.xAxis = {
                ..._chartOption.xAxis,
            ...
            const dataMatrix = valueList.map((item, i) => {
                return data?.map((d) => d[item.fieldKey]);
            });
                    ...
                    valueList.map((item, i) => {
                        return {
                            ...
                            data: data?.map((d, j) => {
                                let _value;
                                try {
                                    _value = Big(isPercentMode ? (d[item.fieldKey] / sumData[j]) * 100 : d[item.fieldKey])
                                            ...
                                            if (isPercentMode) {
                                                try {
                                                    return params.value != null && !isNaN(params.value)
                                                        ? `${numberWithCommas(Big(params.value)
                                                            .round(item.decimalLength || 0)
                                                            .toNumber())}%`
                                                        : '--%';
                                                }
    ...
    const builtOption = buildChartOption(_chartOption, restSettings, isLineStack ? 'line-stack' : 'stack');
    let options = mergeOption(builtOption, userOptions);
    if (setOption) {
        options = setOption(cloneDeep(options));
    }
    return (React.createElement(React.Fragment, null,
        React.createElement(ReactEcharts, { ref: ref, option: { ...cloneDeep(options) }, notMerge: true, opts: { renderer: 'svg' }, style: { height: '100%', width: '100%' }, ...resetOptions })));
};
